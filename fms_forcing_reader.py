...
class reader:
    ...
    def __init__(self, path, lat, lon):
        ...
        self.file = path
        self.op_grp = str(np.round(lat,1)) + '_' + str(np.round(lon,1))
    ...
    def get_profile_mean(self, var, zero_bottom=False):
        ...
        assert (('t','pfull') == var_handle.dimensions or ('pfull',) == var_handle.dimensions)




        if var == 'pfull':
            data = var_handle[:]*100.0
            ps = np.mean(op_grp.variables['ps'], axis=0)
            return np.append(data,ps)[::-1]
        else:
    ...
    def get_timeseries_mean(self, var):
        ...
        var_handle =  op_grp.variables[var]
        assert(('t',) == var_handle.dimensions)
    ...
    def get_interp_profile(self, var, z, zero_bottom=False, filter=True):
        ...
        p_interp1= pchip(z_gcm[:], data[:])#np.interp(z, z_gcm[:], data[:])
        ...
        p_interp2 = pchip(z_interp1, data_interp1)
        return p_interp2(z)
...
if __name__ == "__main__":
    path = '/Users/kpressel/Dropbox/PycharmProjects/scampy/forcing/new_1.00x_default.nc'
    lat = 15.3
    lon = 180.0

    rdr = reader(path, lat, lon)
